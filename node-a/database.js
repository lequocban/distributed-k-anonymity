...
const DB_PATH = process.env.DB_PATH || path.join(__dirname, 'site_a.db');
 ...
 * @param {string} ageGen - generalized age label
 * @param {string} gender
 * @param {string} zipGen - generalized zipcode
 ...
 * @param {number} ageLevel
 * @param {number} zipLevel
 * @returns {Array<{ hash: string, count: number }>}
 */
 ...
 * blacklistedHashes: set of hashed QI keys that were globally suppressed (count < k).
 * @param {number} ageLevel
 * @param {number} zipLevel
 * @param {string[]} blacklistedHashes
 * @returns {Array} anonymized patient records (no sensitive identity data recoverable)
 */
function getAnonymizedRecords(ageLevel, zipLevel, blacklistedHashes) {
  ...
  return patients
    ...
    .map(r => { const { _hash, ...rest } = r; return rest; }); // strip _hash from output
}
